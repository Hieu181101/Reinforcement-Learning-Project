    ...
    def runValueIteration(self):
        ...
        for s in states:
            ...
            if not terminal:
                ...
                diff = abs(current_value_of_state - self.max_Q(s))
                fringe.push(s, -diff)
        ...
        for _ in range(self.iterations):
            ...
            s = fringe.pop()
            self.values[s] = self.max_Q(s)

            for p in predecessors[s]:
                diff = abs(self.values[p] - self.max_Q(p))
                if diff > self.theta:
